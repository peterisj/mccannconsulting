$(document).ready(function(){
	...
	/* uzliekam FB likes count*/
	FB.api('/407380029349847?access_token=684310771637429|lck5VjcTjtLfmvCaXnrFgt0tGt4', function(response) {
		$('.fb-number').html(response.likes);
	});

	$('#worldgroup').bind('click', function(){
		showWorldgroup();
    	clearInterval(animateDesc);
    	return false;
	});

	$('#consulting').bind('click', function(){
		showConsulting();
		clearInterval(animateDesc);
		return false;
	});
	...
	$('#aizrautiba').bind('click', function(){
		showAizrautiba();
		clearInterval(animateValues);
		return false;
	});
	$('#uzticiba').bind('click', function(){
		showUzticiba();
		clearInterval(animateValues);
		return false;
	});
	$('#attistiba').bind('click', function(){
		showAttistiba();
		clearInterval(animateValues);
		return false;
	});
...
//
//Aprakstu animācijas funkcijas
//
function showConsulting() {
	$('.blue-line').removeClass('first');
	$('.blue-line').addClass('last');
	$('#worldgroup-val').removeClass('active');
	$('#consulting-val').addClass('active');
};

function showWorldgroup() {
	$('.blue-line').removeClass('last');
	$('.blue-line').addClass('first');
	$('#consulting-val').removeClass('active');
	$('#worldgroup-val').addClass('active');
};


// apraksta animācija
var animateDesc = window.setInterval(function()
{
	if($('.blue-line').hasClass('first')){
		showConsulting();
	} else {
		showWorldgroup();
	}
...
//
// vērtību animācijas funkcijas
//
function showUzticiba() {
	$('.line-block').removeClass('act-1');
	$('.line-block').addClass('act-2');
	$('.line-block').removeClass('act-3');

	$('#aizrautiba').removeClass('active');
	$('#uzticiba').addClass('active');
	$('#attistiba').removeClass('active');

	$('#aizrautiba-val').removeClass('active');
	$('#uzticiba-val').addClass('active');
	$('#attistiba-val').removeClass('active');
};

function showAttistiba() {
	$('.line-block').removeClass('act-1');
	$('.line-block').removeClass('act-2');
	$('.line-block').addClass('act-3');

	$('#aizrautiba').removeClass('active');
	$('#uzticiba').removeClass('active');
	$('#attistiba').addClass('active');

	$('#aizrautiba-val').removeClass('active');
	$('#uzticiba-val').removeClass('active');
	$('#attistiba-val').addClass('active');
};

function showAizrautiba() {
	$('.line-block').addClass('act-1');
	$('.line-block').removeClass('act-2');
	$('.line-block').removeClass('act-3');

	$('#aizrautiba').addClass('active');
	$('#uzticiba').removeClass('active');
	$('#attistiba').removeClass('active');

	$('#aizrautiba-val').addClass('active');
	$('#uzticiba-val').removeClass('active');
	$('#attistiba-val').removeClass('active');
};


var animateValues = window.setInterval(function()
{
	if($('.line-block').hasClass('act-1')){
		showUzticiba();

	} else if($('.line-block').hasClass('act-2')) {
		showAttistiba();

	} else if($('.line-block').hasClass('act-3')) {
		showAizrautiba();
	}
}, 11000);
